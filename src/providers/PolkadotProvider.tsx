...
import { DEFAULT_CHAIN, CHAINS } from '@/config/chains'
...
export function PolkadotProvider({ children, endpoint }: { children: React.ReactNode; endpoint?: string }){
  ...
  const getInitialEndpoint = () => {
    ...
    if (savedNetwork) {
      const savedChain = Object.values(CHAINS).find((c: any) => c.name === savedNetwork)
      if (savedChain) return (savedChain as any).endpoint
    }
    return DEFAULT_CHAIN.endpoint
  }

  const [currentEndpoint, setCurrentEndpoint] = useState<string>(getInitialEndpoint())
  ...
  useEffect(() => {
    ...
    const connectWithFallback = async () => {
      ...
      const chainConfig = Object.values(CHAINS).find(
        (c) => c.endpoint === currentEndpoint || c.endpoints.includes(currentEndpoint)
      )
